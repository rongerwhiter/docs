...
`open_mqtt_protocol` 옵션을 설정하여 활성화하면 `MQTT` 패킷 헤더가 분석되며 Worker 프로세스의 `onReceive` 이벤트는 매번 완전한 `MQTT` 데이터 패킷을 반환합니다.

Swoole을 MQTT 서버 또는 클라이언트로 사용하여 완전한 사물 인터넷(IOT) 솔루션을 구현할 수 있습니다.

> 완전한 MQTT 프로토콜 분석 및 코루틴 클라이언트는 [simps/mqtt](https://github.com/simps/mqtt) 를 사용할 수 있습니다.

## 프로그램 코드

다음 코드를 `mqttServer.php`에 작성하십시오.
...
$server->set([
    'open_mqtt_protocol' => true, // MQTT 프로토콜 활성화
    'worker_num' => 1,
]);
