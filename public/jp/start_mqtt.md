# MQTT（モノのインターネット）サーバー

`open_mqtt_protocol`オプションを設定して有効にすると、`MQTT`パケットヘッダーが解析されます。Workerプロセスの`onReceive`イベントは、毎回完全な`MQTT`データパケットを返します。

SwooleをMQTTサーバーまたはクライアントとして使用すると、完全なIOTソリューションを実装できます。

> 完全なMQTTプロトコルの解析とコルーチンクライアントには、[simps/mqtt](https://github.com/simps/mqtt) を使用できます

## プログラムコード

以下のコードを`mqttServer.php`に記述してください。
